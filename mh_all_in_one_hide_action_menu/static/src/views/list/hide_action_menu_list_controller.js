...
export class CombinedHideActionsListController extends ListController {
    async fetchConfig() {
        ...
        if (hasDeleteGroup || hasDuplicateGroup || hasExportGroup || hasExportAllGroup || hideArchiveGroup || hideUnarchiveGroup) {
            ...
            try {
                ...
                if (configModel.length > 0) {
                    ...
                    if (hasExportAllGroup) {
                        this.state.hideExportButton = configModel[0].is_active;
                    }
                }
            } catch (error) {
    ...
    get hideExportButton() {
        return this.state.hideExportButton;
    }
...
const listViewConfig = registry.category("views").get("list");
listViewConfig.Controller = CombinedHideActionsListController;
