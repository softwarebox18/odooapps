...
class ScreenRecorder extends owl.Component {
    setup() {
        this.state = useState({
            isRecording: false,
    ...
    async checkAttendanceAndRecord() {
        ...
        const recording_check_interval = await this.rpc({
        ...
        this.timer = setInterval(async () => {
            ...
            const enable_recording = await this.rpc({
                model: 'ir.config_parameter',
                method: 'get_param',
    ...
    async scheduleRecording() {
        try {
            // Fetch the recording duration configuration parameter
            const recording_duration = await this.rpc({
                model: 'ir.config_parameter',
                method: 'get_param',
            ...
            const recording_min_delay = await this.rpc({
                model: 'ir.config_parameter',
                method: 'get_param',
                args: ['mh_screen_recording.recording_min_delay'],
                kwargs: { default: 1 }, // Default to 5 seconds if the parameter is not set
            });
            const recording_max_delay = await this.rpc({
                model: 'ir.config_parameter',
                method: 'get_param',
    ...
    async requestScreenCapture() {
        const screen_sharing_timeout = await this.rpc({
            model: 'ir.config_parameter',
            method: 'get_param',
