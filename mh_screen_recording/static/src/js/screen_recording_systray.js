...
class ScreenRecorder extends Component {
    ...
    async checkAttendanceAndRecord() {
        ...
        // const recording_check_interval = await rpc.query({
        //     model: 'ir.config_parameter',
        //     method: 'get_param',
        //     args: ['mh_screen_recording.recording_check_interval'],
        // }) || 1;  // Default to 1 minute if the parameter isn't set
        const recording_check_interval = await rpc.query({
            model: 'ir.config_parameter',
            method: 'get_param',
            args: ['mh_screen_recording.recording_check_interval'],
            kwargs: { default: 1 }, // Default to 5 seconds if the parameter is not set
        });
        const recording_check_interval_ms = recording_check_interval * 60000; // Default to 1 minute if the parameter isn't set
        this.timer = setInterval(async () => {

            const enable_recording = await rpc.query({
                model: 'ir.config_parameter',
                method: 'get_param',
                args: ['mh_screen_recording.enable_recording']
            });
            ...
            // console.log("bbbbbbbbbbbbbbbbbb", checkedIn, checkedOut);
            if (checkedOut) {
    ...
    async scheduleRecording() {
        try {
            // Fetch the recording duration configuration parameter
            const recording_duration = await rpc.query({
                model: 'ir.config_parameter',
                method: 'get_param',
                args: ['mh_screen_recording.recording_duration'],
                kwargs: { default: 5 }, // Default to 5 seconds if the parameter is not set
            });
            ...
            const recording_min_delay = await rpc.query({
                model: 'ir.config_parameter',
                method: 'get_param',
                args: ['mh_screen_recording.recording_min_delay'],
                kwargs: { default: 1 }, // Default to 5 seconds if the parameter is not set
            });
            const recording_max_delay = await rpc.query({
                model: 'ir.config_parameter',
                method: 'get_param',
                args: ['mh_screen_recording.recording_max_delay'],
                kwargs: { default: 20 }, // Default to 5 seconds if the parameter is not set
            });
    ...
    async checkAttendanceStatus() {
        try {
            const response = await rpc.query({
                model: 'hr.attendance',
                method: 'get_employee_attendance_status',
                args: [this.user.userId],
            });
            return {
    ...
    async requestScreenCapture() {
        const screen_sharing_timeout = await rpc.query({
            model: 'ir.config_parameter',
            method: 'get_param',
            args: ['mh_screen_recording.screen_sharing_timeout'],
            kwargs: { default: 10 }, // Default to 5 seconds if the parameter is not set
        });
        // const screen_sharing_timeout = await this.getConfigParam('mh_screen_recording.screen_sharing_timeout', 10); // Default is 5 seconds
...
SystrayMenu.Items.push(ScreenRecorder);
